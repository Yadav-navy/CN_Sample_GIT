...
def get_skills(text,skills):
    ...
    ft = [w for w in tokens if w.lower() not in sw]
    ...
    for token in ft:
        if token.lower().replace("_", " ") in [skill.lower().replace("_", " ") for skill in skills]:
            fs.add(token)
    
    for ngram in n_grams:
        if ngram.lower().replace("_", " ") in [skill.lower().replace("_", " ") for skill in skills]:
            fs.add(ngram)
...
def extract_education_from_resume(doc):
    ...
    for entity in doc.ents:
        ...
        #if entity.label_ in ["ORG", "EDU", "DEGREE", "COURSE", "CERTIFICATION"]:
            universities.append(entity.text)

    return universities

def extract_major(doc,majorscsv):
    major_keywords = load_keywords(majorscsv)
